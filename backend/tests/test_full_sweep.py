...
def _resolve_base_url():
    ...
    file_url = (
        (dotenv_values(frontend_env).get("REACT_APP_BACKEND_URL") or "").strip()
...
def _creds():
    ...
    if not creds_path.exists():
        pytest.skip(
            "full sweep requires memory/test_credentials.md", allow_module_level=True
        )
...
def player():
    ...
    r = s.post(
        f"{API}/auth/register",
        json={"email": email, "password": "QaPass123!", "name": "TEST_QA Player"},
        timeout=30,
    )
    if r.status_code != 200:
...
class TestAuth:
    ...
    def test_brute_force_lockout(self):
        ...
        requests.post(
            f"{API}/auth/register",
            json={"email": email, "password": "QaPass123!", "name": "TEST_BF"},
            timeout=30,
        )
        codes = []
        for _ in range(6):
...
class TestSlots:
    ...
    def test_unknown_slot_404(self):
        assert (
            requests.get(f"{API}/games/slots/not_a_slot_xyz", timeout=30).status_code
            == 404
        )
    ...
    def test_spin_flagship_debits_balance(self, player):
        ...
        r = player.post(
            ...
            json={"machine_id": "solar_vanguard", "bet": 20},
            timeout=60,
        )
        ...
        assert (
            abs(d["balance"] - expected) < 0.01
        ), f"before={before} after={d['balance']} win={d.get('win')}"
    ...
    def test_spin_basic_slot(self, player):
        r = player.post(
            f"{API}/games/slots/spin",
            json={"machine_id": "mega_moolah", "bet": 20},
            timeout=60,
        )
        assert r.status_code == 200, r.text[:300]
    ...
    def test_spin_rejects_bad_bet(self, player):
        r = player.post(
            f"{API}/games/slots/spin",
            json={"machine_id": "solar_vanguard", "bet": -5},
            timeout=30,
        )
    ...
    def test_spin_rejects_unknown_machine(self, player):
        r = player.post(
            f"{API}/games/slots/spin",
            json={"machine_id": "nope_xyz", "bet": 20},
            timeout=30,
        )
    ...
    def test_spin_rejects_over_balance(self, player):
        r = player.post(
            f"{API}/games/slots/spin",
            json={"machine_id": "solar_vanguard", "bet": 99_999_999},
            timeout=30,
        )
...
class TestKeno:
    ...
    def test_play(self, player):
        ...
        r = player.post(
            ...
            json={"picks": [1, 5, 9, 14, 22], "stake": 25},
            timeout=60,
        )
        assert r.status_code == 200, r.text[:300]
        d = r.json()
    ...
    def test_duplicate_picks_are_deduped(self, player):
        r = player.post(
            f"{API}/games/keno/play", json={"picks": [1, 1, 1], "stake": 25}, timeout=30
        )
        assert r.status_code == 200, r.text[:200]
    ...
    def test_play_rejects_out_of_range(self, player):
        r = player.post(
            f"{API}/games/keno/play", json={"picks": [0, 99], "stake": 25}, timeout=30
        )
        assert r.status_code in (400, 422), r.status_code
    ...
    def test_wheel_spin(self, player):
        ...
        if r.status_code == 200:
            ...
            assert r2.status_code in (
                400,
                429,
            ), f"daily wheel not rate limited: {r2.status_code}"
    ...
    def test_withdraw_blocked_without_kyc(self, player):
        r = player.post(
            ...
            json={
                "currency": "usd",
                "amount": 20,
                "destination": "TEST_dest_acct_1234",
            },
            timeout=30,
        )
        assert r.status_code in (
            ...
            402,
            403,
        ), f"withdrawal not KYC-gated: {r.status_code} {r.text[:200]}"
...
class TestAdminHQ:
    ...
    def test_hq_inbox_wrong_pin(self, admin):
        r = admin.get(
            f"{API}/support/tickets", headers={"X-HQ-Pin": "0000"}, timeout=30
        )
    ...
    def test_support_ticket_appears_in_hq(self, admin):
        ...
        c = requests.post(
            ...
            json={
                ...
                "email": "qa@test.com",
                "subject": subject,
                "message": "Automated sweep ticket",
            },
            timeout=30,
        )
        ...
        time.sleep(0.5)
        r = admin.get(
            f"{API}/support/tickets", headers={"X-HQ-Pin": HQ_PIN}, timeout=30
        )
        assert (
            r.status_code == 200
        ), f"HQ inbox with valid PIN failed: {r.status_code} {r.text[:200]}"
        rows = r.json()
        ...
        res = admin.post(
            f"{API}/support/tickets/{tid}/resolve",
            headers={"X-HQ-Pin": HQ_PIN},
            timeout=30,
        )
    ...
    def test_support_message_bot(self, player):
        r = player.post(
            f"{API}/support/message", json={"message": "How do I withdraw?"}, timeout=60
        )
        assert r.status_code == 200
