...
const TXN_LABEL = {
  slots: "Slots",
  keno: "Warhead Keno",
  coinflip: "Dog-Tag Flip",
  daily_bonus: "Daily Supply Drop",
  signup_bonus: "Enlistment Bonus",
  deposit: "Credit Resupply",
  free_spin: "Free Fire Spin",
    ...
    >
      ...
      >
        ...
        <div className="flex-1">
          <h3 className="font-display text-2xl tracking-wide gold-gradient leading-none">
            REAL-MONEY CASHIER IS LIVE
          </h3>
          <p className="text-sm text-foreground/80 mt-2 leading-relaxed">
            Deposit &amp; withdraw real funds via card (Stripe), crypto
            (NOWPayments) and the approval vault at{" "}
            <span className="text-gold">wages-vault.emergent.host</span>. Credit
            packages below remain play-money for gameplay.
          </p>
          <Link
            to="/cashier"
            data-testid="wallet-goto-cashier"
            className="inline-flex items-center gap-2 mt-3 font-stencil tracking-widest uppercase text-sm text-black bg-gold px-4 py-2 hover:bg-gold/90"
          >
            Open Cashier Vault
          </Link>
        </div>
      ...
      <div className="hud hud-gold p-6 flex flex-wrap items-center justify-between gap-4 mb-10">
        ...
        <img
          src={BRAND.emblem}
          alt="emblem"
          className="w-16 h-16 rounded-full ring-1 ring-gold/40 object-cover"
        />
      </div>

      {cashback && cashback.percent > 0 && (
        <div
          data-testid={WALLET.cashback}
          className="hud p-6 flex flex-wrap items-center justify-between gap-4 mb-10"
        >
          <div className="flex items-center gap-4">
            <Percent size={30} weight="fill" className="text-nvg" />
            <div>
              <p className="font-mono text-xs tracking-widest text-nvg/70">
                {cashback.tier} WEEKLY CASHBACK · {cashback.percent}%
              </p>
              <div className="font-display text-3xl tracking-wide text-foreground leading-none mt-1">
                {cashback.available ? (
                  <span className="gold-gradient">
                    +{fmt(cashback.amount)} READY
                  </span>
                ) : (
              ...
              <p className="font-mono text-[11px] text-muted-foreground mt-1">
                {cashback.available
                  ? "Cashback on your weekly net losses is ready to bank."
                  : cashback.seconds_left > 0
                    ? `Next payout in ${Math.ceil(cashback.seconds_left / 3600)}h`
                    : "Play more to accrue cashback on net losses."}
              </p>
            </div>
          </div>
          <Button
          ...
          <div className="hud divide-y divide-border">
            {cashbackLog.map((c) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-stencil tracking-wide text-foreground uppercase text-sm">
                      {c.meta?.tier || "VIP"} Cashback
                      {c.meta?.percent ? ` · ${c.meta.percent}%` : ""}
                    </div>
                ...
                <div className="font-mono text-lg text-nvg">
                  +{fmt(c.amount)}
                </div>
              </div>
            ))}
      ...
      <div className="mb-6">
        ...
        <p className="text-muted-foreground text-sm mt-1 flex items-center gap-1">
          <ShieldCheck size={14} className="text-gold" /> Secure Stripe checkout
          (test mode). Play-money credits — no cash value.
        </p>
      </div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-14">
        {packages.map((p) => {
          ...
          return (
            <div
              key={p.id}
              data-testid={WALLET.pkg(p.id)}
              className="hud p-6 flex flex-col hover:border-gold/60 transition-colors"
            >
              <div className="flex items-center justify-between">
                <Package size={28} weight="duotone" className="text-nvg" />
                {p.bonus > 0 && (
                  <span className="font-mono text-[10px] text-black bg-gold px-2 py-0.5">
                    +{fmt(p.bonus)} BONUS
                  </span>
                )}
              </div>
              ...
              >
                ...
                {busy === p.id ? "REDIRECTING..." : "RESUPPLY"}
              </Button>
            </div>
          );
        })}
      ...
      <div data-testid={WALLET.txn} className="hud divide-y divide-border">
        {txns.length === 0 && (
          <div className="p-6 font-mono text-sm text-muted-foreground">
            No transactions yet. Deploy into the lobby.
          </div>
        )}
        {txns.map((t) => {
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                {t.type === "daily_bonus" || t.type === "signup_bonus" ? (
                  <Gift size={18} className="text-nvg" />
                ) : positive ? (
                  <ArrowUp size={18} className="text-nvg" />
                ) : (
                  <ArrowDown size={18} className="text-alert" />
                )}
                <div>
                  <div className="font-stencil tracking-wide text-foreground uppercase text-sm">
              ...
              <div
                className={`font-mono text-lg ${positive ? "text-nvg" : "text-alert"}`}
              >
                {positive ? "+" : ""}
                {fmt(t.amount)}
              </div>
