...
export default function CoinFlipGame() {
  ...
  const faceImg =
    face === "heads" ? "/slots/coin_heads.png" : "/slots/coin_tails.png";

  return (
    <div
      data-testid={COINFLIP.root}
      ...
      <div className="max-w-lg mx-auto px-4 sm:px-8 py-8">
        ...
        >
          ...
          <div data-testid={COINFLIP.result} className="h-10 text-center">
            {result && !flip && (
              <p
                className={`font-display text-3xl tracking-wide animate-pop ${result.win > 0 ? "gold-gradient" : "text-alert"}`}
              >
                {LBL[result.outcome]} ·{" "}
                {result.win > 0 ? `+${fmt(result.win)}` : `-${fmt(bet)}`}
              </p>
            )}
          </div>

          <div className="flex gap-3 w-full">
            {sideBtn("heads", COINFLIP.heads)}
            {sideBtn("tails", COINFLIP.tails)}
          </div>
          ...
          <div className="w-full">
            ...
            <input
              ...
              value={bet}
              onChange={(e) =>
                setBet(Math.max(10, Number(e.target.value) || 10))
              }
