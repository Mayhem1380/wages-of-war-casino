...
export default function KycPage() {
  ...
  const saveBanking = async () => {
    ...
    try {
      ...
      toast.error(
        e.response?.data?.detail || "Banking details validation failed",
      );
    }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-8 py-12">
      ...
      <p className="text-sm text-muted-foreground mt-2">
        Upload a government ID and add the exact payout details to match your
        verified identity before withdrawals are approved.
      </p>

      <div className="mt-6 space-y-6">
        <div className="rounded border border-nvg/30 bg-nvg/5 p-4">
          <p className="font-stencil text-xs tracking-[0.3em] text-nvg uppercase">
            Identity verification
          </p>
          <p className="text-sm text-muted-foreground mt-2">
            Verification is handled securely by the connected identity provider.
            No identity documents are uploaded through this page.
          </p>
          <Button
            onClick={startVerification}
            disabled={busy}
            className="mt-4 bg-nvg text-black"
          >
            {busy
              ? "Opening secure verification..."
              : "Start secure identity verification"}
          </Button>
        </div>

        <div className="rounded border border-gold/30 bg-gold/5 p-4 space-y-4">
          <p className="font-stencil text-xs tracking-[0.3em] text-gold uppercase">
            Banking details
          </p>
          <div className="grid md:grid-cols-2 gap-4">
            <input
              ...
              className="bg-black/50 border border-border px-3 py-2 text-sm"
            />
            <input
              name="bank_name"
            ...
            <input
              name="bank_country"
              value={banking.bank_country}
              onChange={handleBankingChange}
              placeholder="Country code (AU/US/GB)"
              className="bg-black/50 border border-border px-3 py-2 text-sm"
            />
            <input
              name="account_number"
            ...
            <input
              name="bsb_code"
              value={banking.bsb_code}
              onChange={handleBankingChange}
              placeholder="BSB code (AU)"
              className="bg-black/50 border border-border px-3 py-2 text-sm"
            />
            <input
              name="routing_number"
            ...
            <input
              ...
              onChange={handleBankingChange}
              placeholder="IBAN"
              className="bg-black/50 border border-border px-3 py-2 text-sm md:col-span-2"
            />
            <input
              name="swift_code"
            ...
            <input
              name="address_line1"
              value={banking.address_line1}
              onChange={handleBankingChange}
              placeholder="Banking address line 1"
              className="bg-black/50 border border-border px-3 py-2 text-sm"
            />
          </div>
        </div>
