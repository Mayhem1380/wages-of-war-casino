...
export default function SlotGame() {
  ...
  const highlight = (data) => {
    ...
    (data.line_wins || []).forEach((lw) =>
      lw.positions.forEach(([r, c]) => cells.add(`${r}-${c}`)),
    );
    ...
    >
        ...
        >
          <div className="relative w-full max-w-2xl overflow-hidden border border-gold/50 bg-[#050805] shadow-2xl shadow-gold/20">
            <img
              ...
              alt={`${media.name} premium mission poster`}
      ...
      <div className="grid lg:grid-cols-[1fr_280px] gap-3 sm:gap-6">
        ...
        >
          ...
          {free && (
            <div
              data-testid={SLOT.freeOverlay}
              className="absolute inset-x-0 top-0 z-20 bg-black/85 border-b-2 border-gold px-5 py-3 flex items-center justify-between animate-pop"
            >
              <div className="flex items-center gap-3">
                ...
                <div className="leading-none">
                  <div className="font-display text-2xl tracking-widest gold-gradient">
                    FREE FIRE
                  </div>
                  <div className="font-mono text-[10px] text-nvg tracking-widest">
                    RISING MULTIPLIER ENGAGED
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-5">
                <div className="text-center">
                  <div
                    data-testid={SLOT.freeSpinsLeft}
                    className="font-mono text-2xl text-nvg leading-none"
                  >
                    {free.spinsLeft}
                  </div>
                ...
                <div className="text-center">
                  <div
                    data-testid={SLOT.freeMultiplier}
                    className="font-display text-3xl leading-none gold-gradient"
                  >
                    ×{free.multiplier}
                  </div>
                  <div className="font-mono text-[9px] text-muted-foreground tracking-widest">
                ...
                <div className="text-center">
                  <div
                    data-testid={SLOT.freeWin}
                    className="font-mono text-2xl text-gold leading-none"
                  >
                    {fmt(free.total)}
                  </div>
                  <div className="font-mono text-[9px] text-muted-foreground tracking-widest">
          ...
          <div className="mt-4 flex items-center justify-between border-t border-border pt-4">
            ...
            <div
              data-testid={SLOT.win}
              className={`font-display text-4xl tracking-wide ${lastWin > 0 ? "gold-gradient animate-pop" : "text-muted-foreground"}`}
            >
        ...
        <div className="space-y-4">
          ...
          <div className="hud p-5">
            ...
            <div className="flex items-center gap-2">
              <button
                data-testid={SLOT.betDec}
                onClick={() => changeBet(-20)}
                disabled={inFree}
                className="w-10 h-10 border border-border hover:border-nvg text-nvg flex items-center justify-center disabled:opacity-40"
              >
            ...
            <div className="grid grid-cols-3 gap-2 mt-3">
              {[100, 500, 2000].map((v) => (
                <button
                  ...
                  onClick={() => setBet(v)}
                  disabled={inFree}
                  className="font-mono text-xs border border-border py-1.5 hover:border-gold hover:text-gold text-muted-foreground disabled:opacity-40"
                >
                  {fmt(v)}
                </button>
              ))}
            ...
            <div className="space-y-2 max-h-[220px] overflow-y-auto pr-1">
              ...
              <div className="flex items-center justify-between border-t border-border pt-2 mt-2">
                <div className="flex items-center gap-1">
                  <Target size={18} weight="fill" className="text-nvg" />
                  <SymbolTile id={machine.scatter} size={22} />
                </div>
                <span className="font-mono text-[11px] text-nvg text-right">
                  3+ scatter →<br />
                  {machine.free_spins} free spins, rising ×
                </span>
              </div>
            </div>
          </div>
