...
export default function KenoGame() {
  ...
  return (
    <div
      ...
      style={{
        backgroundImage:
          "linear-gradient(rgba(6,10,8,0.55), rgba(4,7,5,0.78)), url(/slots/keno_bg.jpg)",
              ...
              {NUMS.map((n) => {
                ...
                return (
                  ...
                  >
                    {picked && !result && (
                      <img
                        src="/slots/keno_warhead.png"
                        alt=""
                        className="absolute inset-0 w-full h-full object-contain p-0.5 pointer-events-none opacity-90"
                      />
                    )}
                    {isHit && (
                      <img
                        src="/slots/keno_blast.png"
                        alt=""
          ...
          <div className="space-y-4">
            <div className="hud p-5">
              ...
              <div className="flex gap-2 mb-3">
                <button
                  ...
                  onClick={quick}
                  className="flex-1 flex items-center justify-center gap-1 border border-border py-2 font-mono text-xs hover:border-nvg text-muted-foreground hover:text-nvg"
                >
                ...
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-1 border border-border py-2 font-mono text-xs hover:border-alert text-muted-foreground hover:text-alert"
                >
                  <Trash size={14} /> CLEAR
                </button>
              </div>
            ...
            <div className="hud p-5">
              ...
              <input
                ...
                value={stake}
                onChange={(e) =>
                  setStake(Math.max(10, Number(e.target.value) || 10))
                }
              ...
              <div className="grid grid-cols-3 gap-2 mt-3">
                {[50, 200, 1000].map((v) => (
                  <button
                    key={v}
                    onClick={() => setStake(v)}
                    className="font-mono text-xs border border-border py-1.5 hover:border-gold hover:text-gold text-muted-foreground"
                  >
                    {fmt(v)}
                  </button>
                ))}
            ...
            <Button
              ...
              onClick={play}
              disabled={busy}
              className="w-full h-14 bg-nvg hover:bg-nvg/90 text-black font-display text-xl tracking-widest glow-nvg gap-2"
            >
              <Lightning size={22} weight="fill" />{" "}
              {busy ? "DROPPING..." : "LAUNCH DRAW"}
            </Button>
