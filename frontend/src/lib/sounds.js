...
function tone({ freq = 440, dur = 0.15, type = "sine", gain = 0.3, start = 0, glideTo = null, attack = 0.005, release = 0.06, send = 0 }) {
  const c = ensure();
  if (!c || muted) return;
  ...
  if (send > 0 && verb) { const s = c.createGain(); s.gain.value = send; g.connect(s); s.connect(verb); }
  osc.start(t0);
...
function noiseBurst({ dur = 0.2, gain = 0.2, start = 0, hp = 800, lp = null }) {
  ...
  src.connect(filter);
  if (lp) { const l = c.createBiquadFilter(); l.type = "lowpass"; l.frequency.value = lp; filter.connect(l); l.connect(g); }
...
function coinVoice(start = 0, base = 1180, gain = 0.26) {
  ...
  tone({ freq: base * 1.5, dur: 0.1, type: "sine", gain: gain * 0.7, start: start + 0.015, send: 0.4 });
  tone({ freq: base * 2.02, dur: 0.09, type: "sine", gain: gain * 0.4, start: start + 0.03, send: 0.5 });
}

const CHORD = [523, 659, 784, 1046, 1318, 1568];
function arp(notes, step = 0.08, dur = 0.16, gain = 0.28, type = "triangle", send = 0.35, start = 0) {
  notes.forEach((f, i) => tone({ freq: f, dur, type, gain, start: start + i * step, send }));
}
...
export const sfx = {
  ...
  win: () => {
    ...
    tone({ freq: 1568, dur: 0.5, type: "sine", gain: 0.08, start: 0.22, send: 0.6 });
  },

  bigWin: () => {
    arp(CHORD, 0.09, 0.22, 0.3, "triangle", 0.45);
    tone({ freq: 130.8, dur: 0.9, type: "sawtooth", gain: 0.12, start: 0, send: 0.3 });
    tone({ freq: 261.6, dur: 0.9, type: "sawtooth", gain: 0.1, start: 0, send: 0.3 });
    noiseBurst({ dur: 0.5, gain: 0.06, start: 0.15, hp: 2500 });
    for (let i = 0; i < 5; i++) coinVoice(0.4 + i * 0.06, 1200 + i * 60, 0.16);
  },

  jackpot: () => {
    arp([523, 659, 784, 1046, 1318, 1568, 2093], 0.08, 0.26, 0.3, "triangle", 0.5);
    ...
    for (let i = 0; i < 12; i++) coinVoice(0.5 + i * 0.05, 1100 + (i % 5) * 90, 0.14);
  },
  ...
  scatter: () => {
    tone({ freq: 300, dur: 0.75, type: "sawtooth", gain: 0.22, glideTo: 1500, send: 0.4 });
  ...
  holdStart: () => {
    tone({ freq: 110, dur: 1.0, type: "sawtooth", gain: 0.14, glideTo: 440, send: 0.4 });
    arp([392, 523, 659, 784], 0.12, 0.24, 0.26, "triangle", 0.5, 0.15);
    noiseBurst({ dur: 0.6, gain: 0.08, start: 0.1, hp: 1500 });
  },

  wheelSpin: () => {
    tone({ freq: 200, dur: 1.6, type: "sawtooth", gain: 0.12, glideTo: 900, send: 0.3 });
  ...
  promote: () => {
    ...
    tone({ freq: 1046, dur: 0.5, type: "sawtooth", gain: 0.16, start: 0.6, send: 0.5 });
