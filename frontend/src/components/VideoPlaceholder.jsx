...
export function VideoPlaceholder({
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute inset-0 tactical-bg opacity-60 ${ready ? "pointer-events-none" : ""}`}
      />
      <div className="absolute top-3 left-3 flex items-center gap-2 font-mono text-[10px] tracking-widest text-nvg/70">
        <VideoCamera size={14} weight="fill" />{" "}
        {failed ? "LIVE FEED · OFFLINE" : "LIVE FEED · BRIEFING"}
      </div>
      {!ready && (
        <div className="relative z-10 flex flex-col items-center text-center px-6">
          ...
          <p className="font-mono text-[11px] tracking-widest text-muted-foreground mt-3 animate-flicker">
            {failed
              ...
              : "// LOADING SECURE BROADCAST"}
          </p>
        </div>
      )}
      <span
        className={`absolute bottom-3 right-3 border font-mono text-[10px] px-2 py-0.5 ${failed ? "border-alert/50 text-alert" : "border-nvg/50 text-nvg"}`}
      >
        {failed ? "OFFLINE" : "REC ●"}
      </span>
    </div>
