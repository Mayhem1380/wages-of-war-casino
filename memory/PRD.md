...
- PENDING (not wired this session): WOW Keno + Side Keno backend engines drafted in games.py (play_wow_keno, play_side_keno) but NOT exposed via server.py endpoints and NO frontend UI yet — dormant/harmless. Player-facing Command Hub launchpad in lobby also still pending. DEPLOYMENT still needs Emergent Support to force a clean no-cache rebuild to push preview → production.
