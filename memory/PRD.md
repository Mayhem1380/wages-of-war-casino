...
### 02 Sep 2026 — Redeploy readiness + slot grid/denominations/neon/flash pass
- VERIFIED already-live (this fork found them present, not missing): payment caps in cashier.py (`CASHOUT_TIERS`: <$50 deposit → $1,500 max win; $50+ → $10,000) + name-match KYC (bank account_holder must equal registered name, 403) + KYC-approved gate, all enforced in `/api/cashier/withdraw`. Wheel of Wealth already has `$500 MAJOR` segment (server.py WHEEL_OF_WEALTH). 30% treasury reserve live. Fixed Wheel copy "$5 up to $50" → "$5 up to $500" (DailyWheel.jsx).
- LOBBY GRID: audited /lobby live — 145 cards, only 1 broken image (`tile_golden_dynasty.png`, a 1.66MB PNG that lazy-load-flaked). Optimized to 200KB `tile_golden_dynasty.jpg` and repointed all 3 gameMeta.js refs. `/public/assets/slots/` (the "1,600 assets" path in the brief) does NOT exist; real assets live in `/public/slots/` (255 files). Grid now 0 broken.
- DENOMINATION LOCK: added COIN DENOMINATION row (1¢/2¢/5¢/10¢ → bet 20/40/100/200) + `BUY NOW · {cost}` Buy-Feature button to FlagshipSlot.jsx (testids `flagship-denom-*`, `flagship-buy-feature-btn`). Standard SlotGame.jsx already had both. All flagship machines have free_spins so Buy Now shows.
- WIN/LOSE FLASH: new `WinLossFlash.jsx` full-screen flashing WINNER (gold radial + trophy) / LOSE (red vignette) overlay, pointer-events-none & auto-dismiss (win 1.5s, lose 0.85s) so play is never blocked. CSS keyframes in index.css. Wired into SlotGame, FlagshipSlot, and KenoGame (win + lose paths). Verified LOSE flash renders on Sweet Ammo spin.
...
### 06 Sep 2026 — Merged GitHub copilot/main (20 new slots + upgrades) + submarine footer + crash fix
- Discovered the user's "20 new slots + upgrades" were built on the GitHub branch `origin/copilot/main` (not in this project). Merged it into main.
- Resolved conflicts: kept compliant cashier/KYC (payment caps, bank name-match, 30% reserve) from HEAD (server.py, Kyc.jsx = ours); took new slot/UI work (games.py both → golden_dragon + 20 additional slots; Lobby/SlotGame both; deploy.sh/eslint/manifest theirs). Catalog now 168 machines / 166 public.
- Renamed 16 of the 20 new copilot slots to the user's names (Ghost Squadron, Depth Charge Riches, Midnight Marauders, Abyssal Ambush, Silent Strike, Neon Kraken, Ironclad Infantry, Sapphire Siege, Phantom Platoon, Stormbreaker Slots, Golden Grunt, Reaper Reels, Blackout Battalion, Trident Tactical, Oceanic Overlord, Hellfire Harpoon). 4 requested names (Titanium Tundra, Vortex Vanguard, Crimson Circuit, Emerald Guardian) already existed as slots. 4 leftover copilot slots kept (Auric Bastion, Blacksite Bounty, Jade Fireteam, Wildline Warriors).
- CRITICAL FIX: merge left SoundProvider calling soundManager.combatAmbienceStart/Stop but those methods lived on `sfx` (soundManager closes L63, sfx L167-363). This white-screened the ENTIRE app. Added delegating combatAmbienceStart/Stop to soundManager in lib/sounds.js. App restored (verified landing + lobby render, docHeight>0).
