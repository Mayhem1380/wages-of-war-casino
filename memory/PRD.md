...
- QUEUE (new slots user requested, not yet built): Eyes of Fortune, Fire of Villa Street, Epic Lamp, Cash Machine, 777 Crazy, Nek's Blessing, 50 Dragons, 5 Rabbits, Raging Bull, Book of the Fallen, Gold Rush, 5 Lions, Gates of Olympus. Build pattern: add _machine() config to games.py with 4 premium symbols + card ranks + wild/scatter; generate 4 painted symbols + bg + thumb per slot; wire SYMBOL_META img, FLAGSHIP_ART, FLAGSHIP_IDS (both games.py AND gameMeta.js). Confirm "Fire of Villa Street" exact theme with user.
